...
$ServiceConfig = @{
    ...
    "career" = @{
        DatabaseName = "career_service_db"
        ConnectionStringName = "ConnectionStrings__CareerDbContext"
...
try {
    ...
    # Special case for CareerService which uses a different environment variable name
    if ($ServiceName -eq "career") {
        Set-Item -Path "env:CareerDbContext" -Value $connectionString
    }
    ...
    if ($serviceConfig -and $serviceConfig.ConnectionStringName) {
        ...
        # Special cleanup for CareerService
        if ($ServiceName -eq "career") {
            Remove-Item "env:CareerDbContext" -ErrorAction SilentlyContinue
        }
    }
}
