...
/// <summary>
/// Custom value resolver for converting Markdown to HTML
/// </summary>
public class MarkdownToHtmlResolver(IMarkdownService markdownService, string propertyName) : IValueResolver<JobPosting, JobPostingResponse, string>
{
    private readonly IMarkdownService _markdownService = markdownService;
    private readonly string _propertyName = propertyName;

    public string Resolve(JobPosting source, JobPostingResponse destination, string destMember, ResolutionContext context)
    {
        var markdown = _propertyName switch
        {
            nameof(JobPosting.Description) => source.Description,
            nameof(JobPosting.Requirements) => source.Requirements,
            nameof(JobPosting.Responsibilities) => source.Responsibilities,
            _ => string.Empty
        };
...
/// <summary>
/// Custom value resolver for country names
/// </summary>
public class CountryNameResolver(ICountryServiceClient countryService) : IValueResolver<JobApplication, JobApplicationResponse, string?>
{
    private readonly ICountryServiceClient _countryService = countryService;

    public string? Resolve(JobApplication source, JobApplicationResponse destination, string? destMember, ResolutionContext context)
    {
        if (string.IsNullOrEmpty(source.ApplicantCountryCode))
        {
            return null;
        }

        // This will be called synchronously in mapping - should use async wrapper in service layer instead
        return source.ApplicantCountryCode;
    }
}

/// <summary>
/// Custom value resolver for file URLs
/// </summary>
public class FileUrlResolver(IUploadServiceClient uploadService) : IValueResolver<JobApplication, JobApplicationResponse, string?>
{
